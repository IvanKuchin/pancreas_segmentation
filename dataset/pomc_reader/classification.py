...
class Reader(interface.IReader):
    ...
    def read_data(self, folder:str) -> tuple[npt.NDArray[np.int32], dict]:
        ...
        self.mask = self.__read_nii(os.path.join(folder, config.CLASSIFICATION_SEGMENTATION_MASK_FILENAME))
        self.mask[self.mask == config.PANCREAS_ID_IN_MASK] = 1

        return dicom, dicom_metadata
    
    def read_label(self, folder:str) -> tuple[npt.NDArray[np.int32], dict]:
        result = np.array([])
        fname = os.path.join(folder, config.CLASSIFICATION_LABEL_FILENAME)
    ...
    def resacale_if_needed(self, src_data: npt.NDArray[np.float32], label_data: npt.NDArray[np.float32], percentage: int) -> tuple[npt.NDArray[np.float32], npt.NDArray[np.float32]]:
        scaled_data, _ = dicom_nrrd.resacale_if_needed(src_data, self.mask, percentage)
