...
def craft_network(checkpoint_file = None, apply_batchnorm=True):
    downsample_steps = [
        double_conv(32, [3,3,1]),   # (?, 128, 128, __filters__)
        double_conv(64, [3,3,1]),   # (?,  64,  64, __filters__)
        double_conv(128, [3,3,1]),  # (?,  32,  32, __filters__)
        double_conv(256, [3,3,1]),  # (?,  16,  16, __filters__)
    ]
    ...
    for idx, _filter in enumerate(filters):
        x = double_conv(_filter, apply_batchnorm=apply_batchnorm)(x)
